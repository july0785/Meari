...
export interface Config {
  ...
  videoCover?: 'mix' | 'scenery' | 'crop'; // 영상(16:9) 썸네일 처리: 반반(기본) / 풍경만 / 스마트 크롭만
  // 디스코드가 여러 개(안정판·PTB·Canary) 켜져 있을 때 어디에 표시할지.
  // 'auto'(기본: 안정판 > PTB > Canary) | 'all'(전부) | 'stable' | 'ptb' | 'canary' | 디스코드 계정 이름
