...
### ⬇️ [**최신 버전 내려받기 (Releases)**](https://github.com/july0785/Meari/releases/latest)

> 실행 파일(`.exe` / `.zip`)은 위 **Releases** 에 있습니다. (이 코드 목록에는 없습니다 — 100MB가 넘는 실행 파일은 GitHub 코드 저장소에 올릴 수 없어 Releases 로 배포합니다.)
> 처음이면 **설치 버전**을, 백신이 막으면 **`win-x64.zip`** 을 받으세요. → [실행이 안 될 때](#백신v3windows-defender-등-차단-대처)
...
4. **디스코드를 두 개 이상(안정판 + PTB/Canary) 켜 둔 경우** → 기본은 안정판에 표시합니다. 다른 계정으로 로그인된 PTB/Canary 쪽에 떠서 "안 보이는" 것처럼 느껴질 수 있으니, 원하는 쪽을 `config.json` 의 `discordTarget` 으로 지정하세요 (예: `"discordTarget": "my_name"`). 어느 클라이언트가 어느 계정인지는 `node test/discord-clients.mjs` 로 확인할 수 있습니다.
...
| `videoCover` | `"mix"` | 영상(16:9) 썸네일 처리. `"mix"`(기본) = 분위기 태그 일치 → 풍경, 매끈한 사진 썸네일 → 스마트 크롭, 복잡한(글씨·그래픽) 썸네일 → 풍경. `"scenery"` = 항상 풍경, `"crop"` = 항상 스마트 크롭. 풍경 사진은 `scenery/` 폴더에서 관리. |
| `discordTarget` | `"auto"` | 디스코드가 여러 개(안정판·PTB·Canary) 켜져 있을 때 어디에 표시할지. `"auto"` = 안정판 > PTB > Canary 순으로 하나, `"all"` = 전부, `"stable"` / `"ptb"` / `"canary"` = 해당 빌드만, 디스코드 계정 이름(예: `"my_name"`) = 그 계정이 로그인된 클라이언트만. 지정한 대상이 꺼져 있으면 다른 데 붙지 않고 기다립니다. |
